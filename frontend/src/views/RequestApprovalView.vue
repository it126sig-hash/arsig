<template>
  <div class="p-4 md:p-6">
    ...
    <Tabs value="0">
      <TabList>
        <Tab value="0">Perlu Persetujuan Saya</Tab>
        <Tab value="1">Riwayat Request Saya</Tab>
      </TabList>
      <TabPanels>
        <TabPanel value="0">
          <!-- Stats / Summary -->
          ...
          <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            >
                ...
                <template #body="{ data }">
                  <div class="flex flex-col">
                    <div class="flex items-center gap-2">
                      ...
                      <Tag v-if="data.archive?.is_confidential" value="CONFIDENTIAL" severity="warn" class="!text-[9px]" />
                    </div>
              ...
              <Column header="Tahap">
                <template #body="{ data }">
                  <Tag :value="getStageLabel(data)" :severity="getStageSeverity(data)" />
                </template>
              </Column>
              ...
              <Column header="Aksi" style="width: 150px">
                <template #body="{ data }">
                  <div v-if="data.status === 'pending'" class="flex gap-2">
                    <Button icon="pi pi-check" severity="success" text rounded v-tooltip="getApproveTooltip(data)" @click="confirmApprove(data)" />
                    <Button icon="pi pi-times" severity="danger" text rounded v-tooltip="'Tolak'" @click="confirmReject(data)" />
                  </div>
                  <div v-else-if="data.status === 'approved' && data.approval_stage === 'completed' && data.otp_code" class="flex flex-col">
                      <div class="flex items-center gap-2">
        ...
        <TabPanel value="1">
          <!-- Outgoing request history: archives I requested OTP access for -->
            ...
            >
              <template #empty>
                <div class="text-center py-12 text-slate-400">
                  ...
                  <p>Anda belum pernah meminta akses OTP ke arsip manapun.</p>
                </div>
              </template>
              <Column field="id" header="ID" style="width: 70px"></Column>
              <Column header="Arsip">
              ...
              <Column header="OTP" style="width: 160px">
                <template #body="{ data }">
                  <div v-if="data.status === 'approved' && data.approval_stage === 'completed' && data.otp_code" class="flex items-center gap-2">
                    <span class="text-[10px] text-slate-400 uppercase font-bold">{{ data.otp_code }}</span>
                    <Button icon="pi pi-copy" severity="secondary" text rounded size="small" class="!w-6 !h-6" v-tooltip="'Salin OTP'" @click="copyToClipboard(data.otp_code)" />
                  </div>
...
import { fetchArchiveRequests, fetchMyArchiveRequests, approveArchiveRequest, rejectArchiveRequest } from '@/api/archiveRequestApi'
...
const getStageLabel = (data) => {
    if (!data.requires_department_approval) return data.status === 'approved' ? 'Selesai' : 'PIC Approval'
    if (data.approval_stage === 'department') return 'Kepala Departemen'
    if (data.approval_stage === 'completed') return 'Selesai'
    return 'PIC Approval'
}

const getStageSeverity = (data) => {
    if (data.approval_stage === 'department') return 'warn'
    if (data.approval_stage === 'completed') return 'success'
    return 'info'
}

const getApproveTooltip = (data) => {
    if (data.requires_department_approval && data.approval_stage === 'pic') return 'Setujui PIC dan teruskan ke kepala departemen'
    if (data.requires_department_approval && data.approval_stage === 'department') return 'Setujui kepala departemen dan generate OTP'
    return 'Setujui (Generate OTP)'
}
...
const loadMyRequests = async () => {
    ...
    try {
        const res = await fetchMyArchiveRequests()
        myRequests.value = res.data.data
    } catch (err) {
        toast.add({ severity: 'error', summary: 'Gagal', detail: 'Gagal memuat riwayat request Anda.', life: 3000 })
    } finally {
...
const confirmApprove = (data) => {
    const message = data.requires_department_approval && data.approval_stage === 'pic'
        ? `Setujui tahap PIC untuk arsip confidential "${data.archive.name}"? Permintaan akan diteruskan ke kepala departemen dan OTP belum dibuat.`
        : data.requires_department_approval && data.approval_stage === 'department'
            ? `Setujui tahap kepala departemen untuk arsip confidential "${data.archive.name}"? Kode OTP akan dibuat otomatis.`
            : `Apakah Anda yakin ingin MENYETUJUI permintaan akses untuk arsip "${data.archive.name}"? Kode OTP akan dibuat otomatis.`
